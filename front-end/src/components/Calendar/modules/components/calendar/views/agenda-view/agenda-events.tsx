...
import { EventDetailsDialog } from "@/components/Calendar/modules/components/calendar/dialogs/event-details-dialog";
import {
  formatTime,
  getBgColor,
  getColorClass,
  ...
  getFirstLetters,
  toCapitalize,
} from "@/components/Calendar/modules/components/calendar/helpers";
...
export const AgendaEvents: FC = () => {
  ...
  return (
    <Command className="py-4 h-[80vh] bg-transparent scrollbar-hide">
      <div className="mb-4 mx-4">
        <CommandInput placeholder="Type a command or search..." />
      </div>
      <CommandList className="max-h-max px-3 border-t scrollbar-hide">
          ...
          >
            {groupedEvents!.map((event) => (
              <CommandItem
                key={event.id}
                className={cn(
                  "scrollbar-hide mb-2 p-4 border rounded-md data-[selected=true]:bg-bg transition-all data-[selected=true]:text-none hover:cursor-pointer",
                  {
